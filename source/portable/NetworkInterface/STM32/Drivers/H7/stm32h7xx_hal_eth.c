...
/* Includes ------------------------------------------------------------------*/
// markh 15jul25: this seems wrong (including stm32h755xx.h directly rather than via stm32h7xx.h); fixing.
//#include "stm32h755xx.h"

// markh 18jul25: adding this to support our redefinition of HAL_GetTick (at end of file) in terms of
// the FreeRTOS tick counter
#include "FreeRTOS.h"
#include "task.h"
...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef *heth)
{
  ...
  // markh 15jul25: remove extra HAL dependency by accessing SYSCFG directly.
  ...
  volatile uint32_t tmpreg = RCC->APB4ENR;  // safety delay before using SYSCFG
  (void) tmpreg;
  SYSCFG->PMCR = (SYSCFG->PMCR & ~SYSCFG_PMCR_EPIS_SEL) | (4 << SYSCFG_PMCR_EPIS_SEL_Pos); // EPIS=0b100 to choose RMII mode
  ...
  // markh 15jul25: hard-code for a given HCLK, to get 1-us tick (e.g. 300-MHz HCLK -> use 300e6 / 1e6 - 1)
  WRITE_REG(heth->Instance->MAC1USTCR, 300 - 1);
...
/**
  ...
  * @retval HAL status
  */
__attribute__ (( section(".fastcode") ))   // markh added 16oct25
...
// markh 15jul25: if M4 core runs it will be bare-metal without HAL, so this is not needed
...
/* markh 11jul25: if M4 core is used, it will run bare-metal w/o HAL; avoid the HAL_GetCurrentCPUID() call here */
//#if defined(DUAL_CORE)
...
void HAL_ETH_SetMDIOClockRange(ETH_HandleTypeDef *heth)
{
  ...
  hclk = 300000000; // markh 15jul25 (was HAL_RCC_GetHCLKFreq())
...
// markh added 18jul25:
// Redefine HAL_GetTick() in terms of the FreeRTOS 1-ms tick, to avoid pulling in
// more HAL cruft.  Note that xTaskGetTickCountFromISR() is the safest way to access
// FreeRTOS's internal tick counter; if ST knows what they're doing (not always true)
// then they would never use HAL_GetTick() from an interrupt context anyway.
